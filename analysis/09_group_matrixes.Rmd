...
refname_ariba_genefamily<-refname_all_ariba%>%
  select(`AMR Gene Family`, `Drug Class`, `ARO Term`)%>%
  unique()%>%
  as_tibble()%>%
  group_by(`AMR Gene Family`)%>%
...
refname_ariba_drugclass<-refname_all_ariba%>%
  select(`Drug Class`, `AMR Gene Family`, `ARO Term`)%>%
  unique()%>%
  as_tibble()%>%
  separate_rows(`Drug Class`, sep=";", convert = TRUE)%>%
    group_by(`Drug Class`)%>%
...
##Remove duplicate ARO terms in the variable ARO term

y<-refname_ariba_drugclass%>%
  select(`Drug Class`, `ARO Term`)%>%
  separate_rows(`ARO Term`, sep=";")%>%
  group_by(`Drug Class`)%>%
  summarise(`ARO Term` = paste(rle(`ARO Term`)$values, collapse=";"))

refname_ariba_drugclass<-refname_ariba_drugclass%>%
  select(-`AMR Gene Family`, -`ARO Term`)%>%
   full_join(x)%>%
   full_join(y)
```
