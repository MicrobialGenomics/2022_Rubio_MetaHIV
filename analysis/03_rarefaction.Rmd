...
```{r}
rarecurve(t(groot_counts[ ,-1]), step = 1000, sample = c(raremax, 2500), col = "blue", cex = 0.6, ylab = "AMR genes", label = FALSE, xlim=c(0,10000), main=glue("Rarefaction plot at {raremax} counts (Groot data)"))
```
...
```{r}
rarecurve(t(groot_counts[ ,-1]), step = 1000, sample = c(raremax, nrar), col = "blue", cex = 0.6, ylab = "AMR genes", label = FALSE, xlim=c(0,10000), main=glue("Rarefaction plot at {raremax}  and {nrar} counts (Groot data)"))
```
...
Srare <- rarefy(t(groot_counts[ ,-1]), raremax) # observed number of rarefied AMR genes
plot(S, Srare, xlab = "Observed No. of AMR genes", ylab = "Rarefied No. of AMR genes", main=glue("Rarefaction at {raremax} counts"))
abline(0, 1)

Srare <- rarefy(t(groot_counts[ ,-1]), nrar) # observed number of rarefied AMR genes
plot(S, Srare, xlab = "Observed No. of AMR genes", ylab = "Rarefied No. of AMR genes", main=glue("Rarefaction at {nrar} counts"))
abline(0, 1)
```

```{r}
fivenum(nCounts_groot$ncount)
fivenum(nCounts_ariba$counts)

nCounts_all<-nCounts_ariba%>%rownames_to_column(var="sample")%>%rename(ncount_ariba=counts)%>%inner_join(nCounts_groot, by="sample")

cor(nCounts_all$ncount_ariba, nCounts_all$ncount, method = "spearman")
wilcox.test(nCounts_all$ncount_ariba, nCounts_all$ncount)
```
