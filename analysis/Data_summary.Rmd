...
Get medatada file and check data concordance
```{r}
metadata<-read_csv("data/Metadata/metadata.csv")
ariba_samples<-list.files("data/aribaData")%>%str_extract(pattern = "Sample_[:digit:]+")
groot_samples<-list.files("data/grootData")%>%str_extract(pattern = "Sample_[:digit:]+")
all.equal(ariba_samples, groot_samples)
all.equal(ariba_samples, metadata$SampleID) ## One extra sample in metadata file
metadata<-metadata%>%filter(SampleID %in% ariba_samples)
```
...
df_filt<-lapply(list_df, function(x) select(x, ref_name, reads)) ##select variables
...
```{r}
ariba_counts<-transform_data(ariba_data)
