...
##merge data with metadata and reference data    
  ...
  sig_amr<-data_all%>%
    nest(data=-ref_name)%>%
    mutate(test=map(.x=data, ~wilcox.test(value~CD4_hl, data=.x)%>%tidy))%>%
    unnest(test)%>%
...
There are `r nrow(sig_amr)` AMR genes with significantly different RPKM values according to CD4 levels (low/high)
...
There are `r nrow(sig_amr)` AMR genes with significantly different rarefied counts according to CD4 levels (low/high)
